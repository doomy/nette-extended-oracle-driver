...
use Dibi;
use ExtendedOracleDriver\DateConfig;
 ...
 */
class ExtendedOracleDriver implements Dibi\Driver, Dibi\ResultDriver, Dibi\Reflector
{
    use Dibi\Strict;
    ...
    public function connect(array & $config)
    {
        ...
        $this->query("ALTER SESSION SET NLS_TIMESTAMP_FORMAT = '".DateConfig::NLS_TIMESTAMP_FORMAT."'");
        $this->query("ALTER SESSION SET NLS_SORT = 'CZECH_AI'");
    ...
    public function updateBlob($table, $columnName, $binaryData, $id) {
        $sql = "UPDATE $table SET $columnName = EMPTY_BLOB() WHERE id = $id RETURNING $columnName INTO :binaryData";
        $result = oci_parse($this->connection, $sql);
        $blob = oci_new_descriptor($this->connection, OCI_D_LOB);
        oci_bind_by_name($result, ":binaryData", $blob, -1, OCI_B_BLOB);
        oci_execute($result, OCI_DEFAULT) or die ("Unable to execute query");
        if ($blob->save($binaryData)) {
